...
// calc upward radiative flux through the bottom of the cells
{
  ...
  if(params.radiation)
  {
    ...
    k_i.reindex(this->zero) = blitz::first( tmp1(ijk).reindex(this->zero) < params.ForceParameters.q_i, this->vert_idx)(blitz::tensor::i, blitz::tensor::j); // vertical index of first cell above inversion (inversion is at the lower edge of this cell)
    ...
    radiative_flux(ijk).reindex(this->zero) += where(this->vert_idx > k_i.reindex(this->zero),  // works even in 2D ?!?!
        ...
        (0.25 * pow((this->vert_idx - 0.5) * params.dz - (k_i.reindex(this->zero) - .5) * params.dz, 4./3) +
        (k_i.reindex(this->zero) - .5) * params.dz * pow((this->vert_idx - 0.5) * params.dz - (k_i.reindex(this->zero) - .5) * params.dz, 1./3))
      , 0);
  }
  else
