...

  {
    ...
    if(user_params.n1_stp*si::cubic_metres >= 0) {
      rt_params.cloudph_opts_init.dry_distros.emplace(
        libcloudphxx::lgrngn::kappa_soluble_fraction_t<thrust_real_t>(user_params.kappa1, user_params.soluble_fraction1),
    ...
    if(user_params.n2_stp*si::cubic_metres >= 0) {
      rt_params.cloudph_opts_init.dry_distros.emplace(
        libcloudphxx::lgrngn::kappa_soluble_fraction_t<thrust_real_t>(user_params.kappa2, user_params.soluble_fraction2),
        std::make_shared<setup::log_dry_radii<thrust_real_t>> (
          thrust_real_t(1.0e-6) * si::metres,
    ...
    if(user_params.n1_stp*si::cubic_metres < 0 && user_params.n2_stp*si::cubic_metres < 0) {
      rt_params.cloudph_opts_init.dry_distros.emplace(
        libcloudphxx::lgrngn::kappa_soluble_fraction_t<thrust_real_t>(case_ptr->kappa, case_ptr->soluble_fraction),
        std::make_shared<setup::log_dry_radii<thrust_real_t>> (
          case_ptr->mean_rd1,
          case_ptr->mean_rd2,
          case_ptr->sdev_rd1,
...
/*
      ...
      libcloudphxx::lgrngn::kappa_soluble_fraction_t<thrust_real_t>(thrust_real_t(1.28), thrust_real_t(1.)),
